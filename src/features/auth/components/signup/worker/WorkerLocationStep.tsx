...
interface WorkerLocationStepProps {
    navigation: RootStackNavigationProp;
    onNext: (location: { address: string; latitude: number; longitude: number }) => void;
    onBack: () => void;
}

export default function WorkerLocationStep({ onNext, onBack }: WorkerLocationStepProps) {
    ...
    return (
        <SignupLayout onBack={onBack}>
            <View style={styles.container}>
                <Title>
                    <HighlightText>활동지역</HighlightText>을 알려주세요
                </Title>
                <SubTitle>가까운 일자리를 추천해드릴게요. {'\n'}이 위치가 맞으신가요?</SubTitle>
